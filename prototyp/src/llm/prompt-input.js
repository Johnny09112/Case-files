// @ts-check
/**
 * Skládání vstupu pro LLM protokol — čistá logika, BEZ závislosti na Vite
 * (`?raw` import). Vlastní modul ze stejného důvodu jako `system-prompt.js`
 * (viz jeho hlavička): `sim/brana-cestiny.js` běží jako plain `node` proces,
 * kam Vite-only `?raw` import (top-level v `prompt.js`) spadne hned na
 * importu — dřív, než se stihne vyhodnotit cokoli uvnitř.
 *
 * `src/llm/prompt.js` (běžící pod Vite i Vitest) tenhle modul jen re-exportuje;
 * `sim/brana-cestiny.js` importuje přímo odsud. Jeden zdroj logiky, dvě cesty
 * importu — přesně stejný vzorec jako `extractSystemPrompt`/`system-prompt.js`.
 *
 * Formát vstupu je definovaný VÝHRADNĚ v `prompty/protokol.md` (sekce
 * „Formát vstupu") — tenhle modul ho nikde neduplikuje jako text, jen ho
 * generuje z uzlových událostí.
 *
 ...
 * `Object.keys(ctx.jmena)`). Samotné hodnoty (celá jména) se do výstupu
 * nikdy nedosazují.
 */
