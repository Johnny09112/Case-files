...
| **Rozhraní pro označení chyb protokolů (požadavek uživatele 2026-08-02, po 3. běhu brány):** u každého AI protokolu v UI možnost označit „chyba češtiny / stylistika / mechanika" + volitelná poznámka; značky jdou do exportního JSONL a do udržovaného souboru zpětné vazby (návrh: `prompty/zpetna-vazba-protokolu.yaml`), který si protocol-humor-tester povinně čte při každém ladění promptu | kódový repo (Opus/Sonnet) + protocol-humor-tester | **ZADÁNO** — stavět po verdiktu 3. běhu brány; malý rozsah (tlačítka + append), engine netknutý |
